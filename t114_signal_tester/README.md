...
The mobile sends its first probe only after five consecutive fresh fixes have at least six satellites, HDOP no worse than 2.00, and walking-scale speed and position changes. It rejects stale fixes and large jumps. After that first sample, another probe requires at least five meters of movement as well as the five-second rate limit, so a stationary tester does not fill the log with duplicate positions. These thresholds are configurable in `include/survey_config.h`.
